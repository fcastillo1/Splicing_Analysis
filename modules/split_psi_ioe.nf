process split_psi_ioe {
    publishDir "${params.outdir}/suppa/split_psi", mode: 'copy'
    ...
    output:
    path "split_*.psi", emit: split_psi
    ...
    if missing_samples:
        print(f"Advertencia: Muestras faltantes en el archivo PSI: {missing_samples}")
    ...
    for condition in set(sample_to_condition.values()):
        ...
        if samples_in_condition:
            output_file = f"split_{os.path.basename('${psi_file}').replace('.psi', '')}_{condition}.psi"
            ...
            print(f"Advertencia: No se encontraron muestras para la condición {condition} en el archivo PSI.")
    """
}
